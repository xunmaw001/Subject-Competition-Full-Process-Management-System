...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 10px 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>竞赛名称</div>
        <el-input v-model="formSearch.jingsaimingcheng" placeholder="竞赛名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>竞赛类型</div>
        <el-select v-model="formSearch.jingsaileixing" placeholder="请选择竞赛类型" :clearable="true">
          <el-option v-for="(item, index) in jingsaileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>竞赛地点</div>
	  ...
	  <el-button v-if="isAuth('jingsaixinxi','新增')" :style='{"cursor":"pointer","padding":"0px 10px","margin":"0 4px 0 0","borderColor":"#fda100","color":"#fda100","outline":"none","borderRadius":"4px","background":"#fcf2e1","borderWidth":"1px 4px","width":"auto","fontSize":"14px","lineHeight":"42px","borderStyle":"solid","height":"42px"}' type="primary" @click="add('/index/jingsaixinxiAdd')"><i v-if="true" :style='{"color":"#fda100","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 10px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"10px","boxShadow":"0px 2px 6px #e6e6e6","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-if="item.jingsaitupian && item.jingsaitupian.substr(0,4)=='http'" :src="item.jingsaitupian" class="image" />
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.jingsaitupian?item.jingsaitupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">{{item.jingsaimingcheng}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">竞赛时间:{{item.jingsaishijian}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">竞赛地点:{{item.jingsaididian}}</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.jingsaileixing != '') searchWhere.jingsaileixing = this.formSearch.jingsaileixing;
        if (this.formSearch.jingsaididian != '') searchWhere.jingsaididian = '%' + this.formSearch.jingsaididian + '%';
        this.$http.get('jingsaixinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/jingsaixinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
